...
class DMUtils(commands.Cog):
    ...
    async def prize_notify(self, ctx: commands.Context, user: discord.Member, amount: str, proof_url: str):
        ...
        Example: [p]prizenotify @User $50.00 https://imgur.com/link
        """
        embed = discord.Embed(
    ...
    async def staff_message(self, ctx: commands.Context, user: discord.Member, *, message: str):
        """Send a direct message to a user as staff. They can reply to the bot to reach you.
        
        Example: [p]staffmessage @User Hello, please check your open ticket.
        """
        embed = discord.Embed(
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            await message.channel.send("⚠️ I couldn't deliver your message because the staff member's DMs are currently closed.")            await interaction.response.send_message(f"❌ An error occurred: `{e}`", ephemeral=True)
...
class DMUtils(commands.Cog):
    ...
    @commands.command(name="prizepayout")
    @commands.guild_only()
    @commands.admin_or_permissions(manage_messages=True)
    async def prize_payout(self, ctx: commands.Context, user: discord.Member, amount: str, proof_url: str):
        """Notify a user that their prize has been paid out."""
        embed = discord.Embed(
            title="🎉 Prize Payout Confirmation",
            description="Congratulations! Your recent prize has been successfully processed and paid out."
    ...
    @commands.guild_only()
    @commands.admin_or_permissions(manage_messages=True)
    async def contact_user(self, ctx: commands.Context, user: discord.Member, *, message: str):
