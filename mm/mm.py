...
class MMWelcome(commands.Cog):
    ...
    async def send_welcome_board(self, ctx, channel: discord.TextChannel = None):
        """Sends the exact raw layout v2 message payload to the specified channel."""
        target_channel = channel or ctx.channel

        # The exact message data structure provided
        message_data = {
            "flags": 36864,
            "allowed_mentions": {
                "parse": []
        ...
        # Creating a raw Route for the target text channel API endpoint
        route = discord.http.Route("POST", f"/channels/{target_channel.id}/messages")
        
        try:
            # We bypass the standard send and inject the exact JSON block directly into Discord's API
            await self.bot.http.request(route, json=message_data)
            if target_channel != ctx.channel:
                await ctx.send(f"✅ Exact welcome board successfully pushed to {target_channel.mention}!")
        except discord.HTTPException as e:
            await ctx.send(f"❌ Failed to send layout v2 components. Error: `{e.text}`")
